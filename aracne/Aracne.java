...
public class Aracne{
	...
	public static void main(String[] args) throws Exception {
		...
		CmdLineParser.Option optionHparacne = parser.addBooleanOption('h',"hpARACNe");
		CmdLineParser.Option optionGeneNumberInMIthresholding = parser.addIntegerOption('g',"geneNumberInMIthresholding");
		...
		boolean hparacne = false;
		if ((Boolean)parser.getOptionValue(optionHparacne)!=null) {
			hparacne = true;
		}

		Integer geneNumberInMIthresholding;
		try {
			geneNumberInMIthresholding = (Integer)parser.getOptionValue(optionGeneNumberInMIthresholding);
		} catch (Exception e) {
			geneNumberInMIthresholding = 3000;
		}
		if (geneNumberInMIthresholding==null) {
			geneNumberInMIthresholding = 3000;
		}
		...
		if(isThreshold){
			...
			runThreshold(expressionFile,outputFolder,geneNumberInMIthresholding,miPvalue,seed,hparacne);
		}
	...
	private static void runThreshold(File _expressionFile, File _outputFolder, int geneNumberInMIthresholding, double miPvalue, int seed, boolean hparacne) throws NumberFormatException, Exception{
		...
		if (hparacne) {
			DataMatrix dm = new DataMatrix(_expressionFile);

			// Check if the sample size
 			sampleNumber = dm.samples.size(); 
			...
			miThreshold = miCPU.calibrateMIThresholdNA(dm.data,geneNumberInMIthresholding,miPvalue,seed);
		} else {
			...
			// Check if the sample size
			sampleNumber = rankData.get(rankData.keySet().toArray()[0]).length;
	...
	// Single run mode
	private static void runHpAracne(
			File expressionFile,
			File transcriptionFactorsFile,
			File outputFolder, 
			String processId,
